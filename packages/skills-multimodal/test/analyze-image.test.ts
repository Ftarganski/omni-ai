...
import { analyzeImageSkill } from "../src/analyze-image.js";
...
describe("analyzeImageSkill", () => {
  ...
  it("passes image as ContentPart[] to the provider", async () => {
    const ctx = visionCtx();
    await analyzeImageSkill.execute(
      { imagePath: join(tempDir, "image.png"), prompt: "What is this?" },
      ctx
    );
  ...
  it("throws when no image source is provided", async () => {
    const ctx = visionCtx();
    await expect(
      analyzeImageSkill.execute({ prompt: "Describe" } as never, ctx)
    ).rejects.toThrow();
  });
});
